...
In this Chapter, we see a basic example that demonstrates how the blueprint and the dependency graph work: Euclid's proof of the infinitude of primes. Initial Lean and Tex content taken from \url{https://github.com/thefundamentaltheor3m/EPFLDemo2}.
...
\begin{lemma}[Factorial is at least one]
\label{lem:one_le_fac}
\lean{one_le_fac}
\leanok
...
\end{lemma}
\begin{proof}
\proves{lem:one_le_fac}
\leanok
Obvious.
\end{proof}

\begin{lemma}[Factorial-plus-one is at least two]
\label{lem:two_le_fac_add_one}
\lean{two_le_fac_add_one}
\leanok
For every natural number $n$, one has
\[
2 \le n! + 1.
